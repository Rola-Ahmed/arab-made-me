...
export const addFactory = asyncHandler(
    async (req, res, nxt) => {
        const { country } = req.body

        const factoryExisit = await Factory.findOne({ where: { userId: req.user.id } })
        ...
        if (importer) return res.status(401).json({ message: "this user is an importer" })


        console.log("here before");
        const factory = await Factory.create({ ...req.body, userId: req.user.id,emailActivated:true })
...
export const verfiyFactory = asyncHandler(
    async (req, res, nxt) => {
        const factory = await Factory.findByPk(req.params.id)
        if (!factory) return res.json({ message: "factory not found" })
        if (
            !factory.legalDocs || !factory.commercialRegisterationNumber || !factory.taxRegisterationNumber ||
            !factory.qualityCertificates || !factory.emailActivated
        )
            return res.json({ message: "complete data first and activate representative email" })
...
export const updateFactory = asyncHandler(
    async (req, res, nxt) => {
...
export const updateOneImage = crudOps.updateOneInMedia(Factory, 'factory', 'images', FactoryDataTypes.images.max)
export const updateOneLegalDoc = crudOps.updateOneInMedia(Factory, 'factory', 'legalDocs', FactoryDataTypes.legalDocs.max)



export const updateOneQualityCertificate = crudOps.updateOneInMedia(Factory, 'factory', 'qualityCertificates', FactoryDataTypes.qualityCertificates.max)
...
export const productsOfFactory = asyncHandler(
    async (req, res, nxt) => {
        const { id } = req.params
...
export const resendConfirmationMail = crudOps.resendConfirmationMail(Factory, 'factory', 'factory/emailActivation?action=', 'repEmail')
...
export const getPrivateLabelings = crudOps.getAllForFactory(PrivateLabeling, 'privateLabelings')

export const getSPMF = crudOps.getAllForFactory(SpecialManufacturingRequest, 'spmfs')
