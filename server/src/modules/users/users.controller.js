...
export const signup = asyncHandler(async (req, res, nxt) => {
  ...
  return user
    ? res.status(201).json({ message: "done", user, token: newToken })
    : nxt(new Error("email exisits", { status: 409 }));
});
...
export const signin = asyncHandler(async (req, res, nxt) => {
  ...
  if (!userExisit.emailActivated) {
    const token = jwt.sign(
      { id: userExisit.id, email: userExisit.email },
      process.env.SECRET_KEY,
      { expiresIn: "5 m" }
    );
    await sendMail({
    ...
    return res
      ...
      .json({
        message:
          "email is not activated yet, check your inbox to activate your email",
      });
  }
  ...
  console.log(
    userExisit.updatedAt.getTime() / 1000 >
      jwt.verify(token, process.env.SECRET_KEY).iat
  );
...
export const updateUser = asyncHandler(async (req, res, nxt) => {
  if (req.user.email != "akramswilam@outlook.com" && req.body.role == "admin")
    return res.json({ message: "you are not allowed to be an admin" });
  if (req.emailActivated)
    return res.json({ message: "can't update email activation" });
  if (req.body.email) return res.json({ message: "email can't be updated" });

  if (req.body.password && !req.body.oldPassword)
    return res.json({ message: "send old password" });
...
export const forgetPassword = asyncHandler(async (req, res, nxt) => {
...
export const resetPassword = asyncHandler(async (req, res, nxt) => {
...
export const logoutUser = asyncHandler(async (req, res, nxt) => {
  await req.user.update({ logout: true });
  return res.status(200).json({ message: "done" });
});

export const createAdmin = asyncHandler(async (req, res, nxt) => {
...
export const cleanUnActive = asyncHandler(async (req, res, nxt) => {
  await User.destroy({
    where: {
