    ...
    return asyncHandler(
        async (req, res, nxt) => {
            if (_uniqueConstraint) {
                const doc = await Model.findOne({ where: { [_uniqueConstraint]: req.body._uniqueConstraint } })
    ...
    return asyncHandler(
        async (req, res, nxt) => {
            ...
            if (docs && index) {
                ...
                return res.status(202).json({ message: "done", [reqModel]: await Model.findByPk(model.id) })
...
export const resendConfirmationMail = (Model, name, path, email) => {
    return asyncHandler(
        async (req, res, nxt) => {
            ...
            if (model.emailActivated) return res.status(400).json({ messgae: "email activated" })
