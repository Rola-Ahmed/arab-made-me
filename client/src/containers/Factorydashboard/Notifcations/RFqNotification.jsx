...
import RfqNotificationList from "components/Factorydashboard/Notification/RfqNotificationList";
...
export default function RfqNotification() {
  ...
  async function fetchNotifcationData() {
    ...
    try {
      let config = {
        method: "get",
        url: `${baseUrl}/factories/factory/rfqs?size=8&page=${page}&include=importer&include=product`,
        headers: {
      ...
      if (response?.data?.message == "done") {
        setNotificationData((prevData) => [...prevData, ...response.data.rfqs]);
  ...
  async function fetchTotalPageData() {
    ...
    try {
      const getTotalPgResponse = await axios.get(
        `${baseUrl}/factories/factory/rfqs`,
        {
          headers: {
            authorization: isLogin,
          },
