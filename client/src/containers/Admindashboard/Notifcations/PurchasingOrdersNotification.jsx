...
import PurchasingOrdersNotificationList from "components/AdminDashboard/Notification/PurchasingOrdersNotificationList";
...
export default function VisitReqNotification() {
  ...
  async function fetchNotifcationData() {
    ...
    try {
      let config = {
        method: "get",
        url: `${baseUrl}/pos?size=${dataSize}&page=${page}&include=importer`,
      };
  ...
  async function fetchTotalPageData() {
    ...
    try {
      const getTotalPgResponse = await axios.get(`${baseUrl}/pos`);
