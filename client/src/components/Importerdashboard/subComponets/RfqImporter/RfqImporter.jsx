...
export default function RfqImporterDash() {
  ...
  async function fetchImportersData() {
    ...
    try {
      let config = {
        method: "get",
        url: `${baseUrl}/importers/importer/rfqs?size=${pagination?.displayProductSize}&page=${pagination?.currentPage}&formsFilter=${filter?.formsFilter}&sort=${filter?.sort}`,
        headers: {
      ...
      if (response?.data?.message == "done") {
        ...
        const uniqueIds = [
          ...new Set(
            response.data.rfqs
              .map((obj) => obj.factoryId) // Extract all importersIds
        ...
        const uniqueProductIds = [
          ...new Set(
            response.data.rfqs
              .map((obj) => obj.productId) // Extract all importersIds
              .filter((id) => id !== null) // Filter out null values
          ),
        ];
  ...
  const downloadCsv = () => {
    ...
    a.href = url;
    a.download = "answerQuotation.csv";
  ...
  useEffect(() => {
    const fetchDataLenght = async () => {
      try {
        // const response1 = await axios.get(`${baseUrl}/rfqs`);
        const response1 = await axios.get(
          `${baseUrl}/importers/importer/rfqs?formsFilter=${filter?.formsFilter}&sort=${filter?.sort}`,
          {
            headers: {
              authorization: isLogin,
            },
                    ...
                    <div className=" th-1st-title-gap d-flex justify-content-start align-items-center">
                      ...
                      <td className="text-truncate">
                        <div className="">
                          ...
                          {/* <p className="trate-sub-title d-flex">{poItem?.productAverageRate ?? "3.5"}<StarRating averageRating="3.5" /></p> */}
                        </div>
                      </td>
                    </div>
                  </th>
