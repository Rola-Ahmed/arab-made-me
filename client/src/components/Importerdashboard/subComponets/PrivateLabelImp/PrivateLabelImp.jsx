...
export default function PrivateLabelImp() {
  ...
  async function fetchFactoriesData() {
    ...
    try {
      let config = {
        method: "get",
        url: `${baseUrl}/importers/importer/privateLabelings?size=${pagination?.displayProductSize}&page=${pagination?.currentPage}&formsFilter=${filter?.formsFilter}&sort=${filter?.sort}`,
        headers: {
    ...
    const fetchDataLenght = async () => {
      try {
        const response1 = await axios.get(
          `${baseUrl}/importers/importer/privateLabelings?formsFilter=${filter?.formsFilter}&sort=${filter?.sort}`,
          {
            headers: {
              authorization: isLogin,
            },
  ...
  const deleteData = async (itemId) => {
    try {
      let config = {
        method: "delete",
        url: `${baseUrl}/privateLabelings/${itemId}`,
        headers: {
          authorization: isLogin,
        },
      };
                      ...
                      <td className="">
                        <div className="title-text-handler">
                          <p className="trate-title">
                            {poItem?.productName ?? "Private Label Product"}
                          </p>
                ...
                <tr className="row">
                  <div className="col-12  w-100 h-100 my-5 py-5">
                    <div className="text-center">
                      <p className="trate-sub-title ">
                        {/* {errorsMsg ?? "Empty No Private Label"} */}
