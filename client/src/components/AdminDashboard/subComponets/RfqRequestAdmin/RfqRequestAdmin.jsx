...
import RfqNotification from "containers/Admindashboard/Notifcations/RFqNotification";
...
export default function RfqFactoryDash() {
    ...
    const fetchDataLenght = async () => {
      try {
        const response1 = await axios.get(
          `${baseUrl}/rfqs?formsFilter=${filter?.formsFilter}&sort=${filter?.sort}`
        );
        ...
        if (response1?.data?.message === "done") {
          setPagination((prevValue) => ({
            ...prevValue,
            totalPage: Math.ceil(
              (response1.data?.quotationrequests?.length || 0) /
  ...
  async function fetchFactoriesData() {
    ...
    try {
      let config = {
        method: "get",
        url: `${baseUrl}/rfqs?size=${pagination?.displayProductSize}&page=${pagination?.currentPage}&formsFilter=${filter?.formsFilter}&sort=${filter?.sort}&include=importer&include=product`,
        headers: {
      ...
      if (response?.data?.message == "done") {
        setAllAnsRfqData(response.data.quotationrequests);
